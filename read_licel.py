    ...
    if not(os.path.exists(dir_meas)):
        ...
        if len(mfiles) > 0:
            ...
            for k in range(len(mfiles)):
                ...
                if optimize_reading is True:
                    sig = read_body_optimized(channel_info, buffer = buffer, sep = sep)
                else:
...
def read_body_optimized(channel_info, buffer, sep):
    """Reads the information from the raw licel files below the header.
    Blocks are separated by #"""
    ...
    sig_raw_arr = np.full((n_channels, max_bins), np.nan)
    ...
    for j in range(n_channels):
        ...
        # Extract and parse data directly into the array
        sig_raw_arr[j, :nbins] = np.frombuffer(data[nbin_s:nbin_e], dtype='<i4')
